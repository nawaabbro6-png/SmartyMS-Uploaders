...
async def txt_handler(bot: Client, m: Message):
    ...
    credit = f"@Mk_khan001"
    token = f"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..iImf90mFu_cI-xINBv4t0jVz-rWK1zeXOIwIFvkrS0M"
    try:    input: Message = await bot.listen(editable.chat.id)
        with open(x, "r") as f:
            content = f.read()
        content = content.split("\n")
        links = []
    ...
    try:
        ...
        arg = 1
    await editable.edit("**Enter Your 𝗕𝗮𝘁𝗰𝗵 Name or send '/mohit' for extracting Name from your text filename😚.**")
    input1: Message = await bot.listen(editable.chat.id)
    raw_text0 = input1.text
    await input1.delete(True)
    if raw_text0 == '/mohit':
